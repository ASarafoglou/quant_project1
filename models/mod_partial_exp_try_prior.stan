...
parameters {
  ...
  //vector<lower=-0.5, upper=0.5>[T] g_full[I];
  vector<lower=0, upper=1>[T] g[I];
...
model {
  ...
  for (t in 1:T)
    for (i in 1:I)
      log_a[i,t] ~ normal(mu_log_a[t], sigma_mu_log_a[t]);
      
  // for (i in 1:I) g_full[i] ~ multi_normal_cholesky(mu_g, L_Sigma_g); 
  ...
  for (t in 1:T) mu_log_a[t] ~ normal(-3 , 1);
  for (t in 1:T) mu_g[t]     ~ normal(0, 0.2) T[0, 0.5];
