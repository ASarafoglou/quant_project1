...
model {
  ...
  for (t in 1:T) mu_b[t]     ~ normal(mu_b_mean, 0.1); 
  for (t in 1:T) mu_g[t]     ~ normal(0.17, 0.05); 
  ...
  for (t in 1:T) sigma_mu_g[t] ~ inv_gamma(12, 1);
  for (t in 1:T) sigma_mu_b[t] ~ inv_gamma(6, 1);
