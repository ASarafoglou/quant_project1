...
round(apply(samples_processed[, var_names], 2, median), 3) 
hist(samples_processed[, 'mu_g[1]'])
hist(samples_processed[, 'g[1,2]'])
```
...
plot(cperc, modelPredictions(alpha = exp(mu_prior[colnames(mu_prior) == 'mu_log_a.1.']), 
                             ...
                             gamma = abs(mu_prior[colnames(mu_prior) == 'mu_g.1.']), 
                             cperc),
     type = 'l',
     lwd = 2,
     las = 1,
...
for(d in 1:length(quant)){
  ...
  analyzePredictedResponsesR(y_pred=y_pred_dat[[d]][1:2400,]   , predat, testtime = 't1', individual = TRUE)
  analyzePredictedResponsesR(y_pred=y_pred_dat[[d]][2401:4800,], predat, testtime = 't2', individual = TRUE)
}
```
