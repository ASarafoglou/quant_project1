...
matplot(as.numeric(rownames(prop)), prop
        , pch = 19, col = qcols, type='l', lwd = 2, lty=1,
...
CORRECTION: we flip the percentage labels around so we have the vagueness always above the threshold. NOTE: we also flip many so that its vagueness is on the other side of the threshold. This needs to be accounted for when we display and interpret the posterior distribution!

```{r}
dat$percent <- case_when(
  dat$qq %in% quant_pos  ~ dat$percent,
  dat$qq %in% quant_neg  ~ 100 - dat$percent,
  dat$qq %in% quant_many ~ 100 - dat$percent
)
  
dat$resp <- case_when(
  dat$qq %in% quant_pos ~ as.numeric(factor(dat$response)) - 1,
  dat$qq %in% quant_neg ~ as.numeric(factor(dat$response)) - 1,
  dat$qq %in% quant_many ~ -as.numeric(factor(dat$response)) + 2
)
